...
class QNetwork:
    def __init__(self, hidden_size, learning_rate, item_num, state_size, pretrain, item_features, name='DQNetwork', lam = 0.2):
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            self.w_item = tf.compat.v1.layers.dense(
                self.item_features, self.hidden_size, activation=None
            )

            self.phi_hat = tf.matmul(self.states_hidden, self.w_item, transpose_b=True)

            self.output3 = (1 - self.lam) * self.output2 + self.lam * self.phi_hat
            ...
            ce_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.actions, logits=self.output3)
...
if __name__ == '__main__':
    ...
    item_features = pd.read_csv("one_hot_encoded.csv").values.astype('float32')
    ...
    QN_1 = QNetwork(name='QN_1', hidden_size=args.hidden_factor, learning_rate=args.lr, item_num=item_num,
                    state_size=state_size, item_features = item_features, pretrain=False)
